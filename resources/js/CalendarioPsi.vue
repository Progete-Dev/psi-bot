			...
			<div class="p-4 max-w-xl mx-auto relative absolute left-0 right-0 overflow-auto mt-10">
				...
				<div class="shadow w-full rounded-lg bg-secondary overflow-hidden w-full block p-8">
					...
					<div class="mb-4">
						...
						<input class="bg-primary appearance-none border-2 border-gray-200 rounded-lg w-full py-2 px-4 text-secondary leading-tight focus:outline-none focus:bg-primaryfocus:border-blue-500" type="text" v-model="event.title">
					</div>

					<div class="mb-4">
						<label class="text-primary  block mb-1 font-bold text-sm tracking-wide">Data</label>
						<input class="bg-primary appearance-none border-2 border-gray-200 rounded-lg w-full py-2 px-4 text-secondary leading-tight focus:outline-none focus:bg-primaryfocus:border-blue-500" type="text" v-model="event.date" readonly>
					</div>

					<div class="mt-8 flex justify-end">
						<button type="button" class="flex bg-primaryhover:bg-secondary text-secondary font-semibold  border border-gray-300 rounded-lg shadow-sm px-2 py-2 mx-2" @click="showEventModal = false">
							Cancelar
						</button>	
						<button type="button" class="flex bg-gray-800 hover:bg-gray-700 text-button font-semibold  border border-gray-700 rounded-lg shadow-sm px-2 py-2 mx-2" @click="showEventModal = false">
							Salvar
						</button>	
					</div>
				</div>
