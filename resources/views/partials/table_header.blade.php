<div class="flex m-2 bg-secondary rounded-md align-baseline">
    <div class="pl-2 mr-2 mt-1">
        <svg class="fill-current   text-secondary w-6 h-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
            <path d="M16.32 14.9l5.39 5.4a1 1 0 0 1-1.42 1.4l-5.38-5.38a8 8 0 1 1 1.41-1.41zM10 16a6 6 0 1 0 0-12 6 6 0 0 0 0 12z" />
        </svg>
    </div>
    <input
        x-model.debounce.500="searchValue"
        class="w-full rounded-md bg-primary text-secondary leading-tight focus:outline-none py-2 px-2"
        id="search" type="text" 
        placeholder="Search">
</div>
<form action="{{$route}}" method="GET">
    <div class="md:flex mx-2 px-2 justify-between">
        ...
        <button name="orderBy" value="{{$option['value']}}" type="submit" class="mx-2 rounded-lg inline-flex items-center {{$option['value'] == $orderBy ? 'bg-primary' : 'bg-theme' }} hover:text-blue-500 focus:outline-none focus:shadow-outline text-gray-500 font-semibold py-2 px-2 md:px-4">
        ...
        <div class="shadow rounded-lg flex justify-center  align-baseline">
            <div class="relative">
                <button @click.prevent="open = !open" class="px-2 rounded-lg inline-flex items-center bg-primaryhover:text-blue-500 focus:outline-none focus:shadow-outline text-gray-500 font-semibold py-2 px-2 md:px-4">
                ...
                <div x-show="open" @click.away="open = false" class="z-40 fixed  bg-secondary rounded-lg shadow-lg md:mt-2 md:mr-2 block py-1 right-0 mr-10">
                    <h3 class="px-2 text-gray-600 text-xl border-b border-gray-400">
                            Status 
                    </h3>
                    @foreach($filterOptions as $index => $options)                        
                        <label class="flex justify-start items-center text-truncate hover:bg-secondary px-4 py-2">
                            ...
                            <div class="select-none text-secondary">{{$options['name']}}</div>
                        </label>
                    @endforeach
                    <div class="flex justify-end">
                        <button type="submit" class="flex bg-secondary hover:text-secondary focus:outline-none  text-button font-semibold  border border-gray-700 rounded-lg shadow-sm px-4 py-2 m-2">
                            ok
                        </button>
                    </div>
                </div>
