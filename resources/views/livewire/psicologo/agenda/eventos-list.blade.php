<div>
    <ul wire:loading.class="animate-pulse" class="flex justify-start overflow-x-auto p-2 w-full mb-2">
        @foreach($events as $horario)
            <li wire:click="dispatchEventAction({{$horario['id']}},{{$loop->index}})"  class=" cursor-pointer select-none block hover:bg-secondary focus:outline-none focus:bg-secondary transition duration-150 ease-in-out bg-primary border border-gray-300 m-2 rounded-md">
