...
@include('partials.page_header')
    ...
    <div x-data="{searchValue : '', orderBy: '',order: 'ASC', open:false}">
        ...
        <li @click="$dispatch('atendimento-modal',{atendimento : {{json_encode($atendimento)}} })" x-show="$refs[{{$atendimento->id}}].innerText.toLowerCase().search(searchValue.toLowerCase()) != -1" class="border rounded-md my-2 shadow-md bg-gray-50 border-indigo-300 hover:bg-gray-100">
    ...
    <div x-data="{openEventModal: false ,atendimento : null }" @atendimento-modal.window="atendimento = $event.detail.atendimento; openEventModal = true;" style=" background-color: rgba(0, 0, 0, 0.8)" class="fixed overflow-auto  z-40 top-0 right-0 left-0 bottom-0 h-full w-full" x-show.transition.opacity="openEventModal">
        <div class="p-4 max-w-3xl mx-auto relative absolute left-0 right-0 overflow-auto mt-10">
            ...
            <div class="shadow w-full rounded-lg bg-white overflow-hidden w-full block p-8">
                ...
                <h2 x-show="atendimento != null" class="font-bold text-2xl mb-6 text-gray-800 border-b pb-2" x-text="'Detalhes Atendimento - ' +(new Date((atendimento != null ? atendimento.data_atendimento : '')).toLocaleString('pt-br'))">Detalhes Atendimento</h2>
